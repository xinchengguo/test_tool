...
/* 过压触发1 */
#define OV_CHECK_1_PORT GPIOB
#define OV_CHECK_1_PIN GPIO_PIN_4
...
/*ADC通道*/
#define T1_DE ADC1_CHANNEL_5

#define Vadc_20V ADC1_CHANNEL_6
/*函数声明*/
  ...
  Next: //循环
    ...
    if (GPIO_ReadInputPin(GPIOA, GPIO_PIN_3) == 0)
    {
      ...
      if (GPIO_ReadInputPin(GPIOA, GPIO_PIN_3) == 0)
      {
        ...
        //计算前20节电芯的电压
        ADValue = 0;
        ADValue = ReadADCValue(Vadc_20V);
        ADValue = ADValue * 5000 / 1024;
        //电压在1700和1900之间 ,,继续
        if ((ADValue > 1650) && (ADValue < 1900)) //前200节电芯电压正常
        {
          BeepFlash(2, 300);
          LED2ON;
        }
        ...
        OV_CHECK_1_ON;
        DelayMs(2000);
        if (GPIO_ReadInputPin(GPIOD, GPIO_PIN_3) == RESET) //前5节电芯过压保护正常
        {
          ;
        }
        else
        ...
        OV_CHECK_2_ON;
        DelayMs(2000);
        if (GPIO_ReadInputPin(GPIOD, GPIO_PIN_3) == RESET) //后5节电芯过压保护正常
        {
           OV_CHECK_1_OFF;
     OV_CHECK_2_OFF;
          BeepFlash(2, 300);
